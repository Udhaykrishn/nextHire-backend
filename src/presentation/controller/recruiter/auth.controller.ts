...
import type {
	RecruiterLoginDto,
	RecruiterLoginResponseDto,
} from "@/application/dto/auth/recruiter/login";
import {
...
export class AuthRecruiterController {
	constructor(
		...
		private readonly _refreshTokenUseCase: IExecutable<
			string,
			RecruiterRefreshTokenDto
		>,

		@Inject(AUTH_RECRUITER_TOKEN.RECRUITER_VERIFY_OTP_USE_CASE)
		private readonly __verifyOtp: IExecutable<
			VerifyOTPDto,
	...
	async signup(
		@Body() signupDto: RecruiterSignupDto,
	): Promise<RecruiterSignResponseDto> {
