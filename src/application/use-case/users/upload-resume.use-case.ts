...
@Injectable()
export class UploadResumeUseCase implements IExecutable<UploadResumeInput, ResponseUserDto> {
	constructor(
	...
	async execute(input: UploadResumeInput): Promise<ResponseUserDto> {
		...
		if (user.resume_url?.key) {
			try {
				await this._s3Service.deleteFile(user.resume_url.key);
			} catch (error) {
		...
		user.changeResumeUrl(fileInfo.key, fileInfo.url);
