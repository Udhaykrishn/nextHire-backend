	...
	implements IExecutable<VerifyOTPDto, VerifyResponseOTPDto>
{
	...
	async execute(dto: VerifyOTPDto): Promise<VerifyResponseOTPDto> {
		...
		const recruiterData = this._recruiterMapper.toDomain(
			JSON.parse(pendingData),
		);

		const storedOtp = await this._redisService.get(
			REDIS_KEYS.OTP.concat(recruiterData.email).concat(
				":",
				USER_ROLE.RECRUITER,
			),
		);
		...
		const createdRecruiter =
			await this._recruiterRepository.save(recruiterData);
		...
		await this._redisService.del(
			REDIS_KEYS.VERIFY_OTP.concat(recruiterData.email).concat(
				":",
				USER_ROLE.RECRUITER,
			),
		);
