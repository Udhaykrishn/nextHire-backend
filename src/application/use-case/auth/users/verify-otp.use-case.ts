...
import { COMMON_TOKEN, USER_MAPPER, USERS_TOKEN } from "@/application/enums/tokens";
...
@Injectable()
export class VerifyUserOtpUsecase implements IExecutable<VerifyOTPDto, VerifyResponseOTPDto> {
	constructor(
	...
	async execute(dto: VerifyOTPDto): Promise<VerifyResponseOTPDto> {
		...
		const otp = await this._redisService.get(REDIS_KEYS.OTP.concat(userData.email));
