...
export class RecruiterChangePasswordUseCase
	implements IExecutable<{ recruiterId: string; dto: ChangePasswordDto }, ResponseRecruiterDto>
{
	constructor(
	...
	async execute(input: { recruiterId: string; dto: ChangePasswordDto }): Promise<ResponseRecruiterDto> {
		...
		const isCurrentPasswordValid = await this._passwordHasher.compare(recruiter.password, dto.currentPassword);

		if (!isCurrentPasswordValid) {
			throw new BadRequestException(PASSWORD_MESSAGES.INVALID_CURRENT_PASSWORD);
		}

		const isSameAsOld = await this._passwordHasher.compare(recruiter.password, dto.newPassword);
		...
		const updatedRecruiter = await this._recruiterRepository.findByIdAndUpdate(recruiter.id as string, recruiter);
