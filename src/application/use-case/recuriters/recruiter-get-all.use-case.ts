...
@Injectable()
export class GetAllRecruitersUseCase
	implements
		IExecutable<PaginationDto, PaginationResponse<ResponseRecruiterDto>>
{
	constructor(
	...
	async execute(
		...
		const recruiters =
			await this._recruiterRepository.findAllRecruiters(paginationDto);

		if (!recruiters || recruiters.data.length === 0) {
			throw new NotFoundException(RECRUITER_MESSAGES.RECRUITER_NOT_FOUND);
		}

		const mappedRecruiters = recruiters.data.map((recruiter) =>
			this._mapper.toResponse(recruiter),
		);
