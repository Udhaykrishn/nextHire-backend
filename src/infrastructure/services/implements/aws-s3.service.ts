...
export class S3Service implements IS3Service<FileInfo, Express.Multer.File> {
	...
	async getSignedUrlForRead(key: string, expiresIn = 3600): Promise<string> {
		return getSignedUrl(this.s3Client, new GetObjectCommand({ Bucket: this.bucket, Key: key }), { expiresIn });
	}

	async getSignedUrlForWrite(key: string, expiresIn = 3600): Promise<string> {
		return getSignedUrl(this.s3Client, new PutObjectCommand({ Bucket: this.bucket, Key: key }), { expiresIn });
	}
}
